			...
			[
				{
					...
					author: 'JohnDoe',
					tasks: ['Sum All Numbers in a Range', 'Sorted Union', 'Seek and Destroy', 'Diff Two Arrays'],
					followers: ['JaneDoe', 'GilesCorey', 'AaronTurner'],
				},
				{
					name: 'Strings',
					...
					followers: ['JakeBlake', 'JohnDoe', 'KimGordon', 'DavidYow'],
				},
				{
					name: 'Morse Code',
					author: 'JaneDoe',
					tasks: ['Decode Morse I', 'Decode Morse II', 'Decode Morse III'],
					followers: ['SteveAlbini', 'JustinBroadrick'],
				},
				{
					name: 'Fibonacci Pack',
					avatar: 'https://codetrainer-images.s3.amazonaws.com/4bf8b705-5f6d-48b0-ac18-4d030467e5cb.svg',
					author: 'JaneDoe',
					tasks: ['Fibonacci numbers', 'Memoized Fibonacci', 'The Millionth Fibonacci'],
					followers: [],
				},
				...
				tasks: (
					await Promise.all(tasks.map(async (name) => tasksRepository.findOne({ where: { name } })))
				).filter((task) => task) as Task[],
				followers: (
					await Promise.all(followers.map(async (username) => usersRepository.getByUsername(username)))
